...
function LaunchWindowContent() {
	...
	useEffect(() => {
		const cleanupSelect = window.electronAPI.onGlobalShortcutSelectSource?.(() => {
			beginInteractiveHudAction();
			requestOpen("sources");
		});
		const cleanupToggle = window.electronAPI.onGlobalShortcutToggleRecording?.(() => {
			if (recording || hasSelectedSource || platform === "linux") {
