			...
			try {
				...
				if (
					...
					return {
						success: false,
						message:
							"Cannot record Screendance App windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"Screendance App needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Screendance App is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Screendance App needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Screendance App is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
