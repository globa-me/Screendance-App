...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `Screendance App ${payload.version} is available`;
		case "downloading":
			return `Downloading Screendance App ${payload.version}`;
		case "ready":
			return `Screendance App ${payload.version} is ready`;
		case "error":
			return `Screendance App ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart Screendance App.";
		case "downloading":
			return "Screendance App is downloading the update and will restart when it is ready.";
		case "ready":
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Screendance App";
